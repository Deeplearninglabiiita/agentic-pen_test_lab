...
THREAT_INTEL_KB = [
    {"id": "cve-2024-001", "text": "CVE-2024-001: Werkzeug 2.3.0 has a path traversal vulnerability. CVSS 7.5. Patch: upgrade to 2.3.1."},
    {"id": "mitre-t1190",  "text": "MITRE T1190: Exploit Public-Facing Application. Adversaries exploit weaknesses in internet-facing software for initial access."},
    {"id": "ioc-185",      "text": "IOC: 185.220.101.5 associated with Cobalt Strike C2 infrastructure. Last seen: 2024-11. Confidence: High. Source: ThreatFox."},
    {"id": "sqli-guide",   "text": "SQL injection via UNION-based extraction: payloads like ' UNION SELECT username,password FROM users-- allow data exfiltration."},
]

# Read query from GUI env var or use default
DEFAULT_QUERY = "We found a connection to 185.220.101.5 from our Flask web server. Is it compromised?"
QUERY = os.environ.get("GUI_RAG_QUERY", DEFAULT_QUERY).strip()
...
Available intel:\n{context}
Task: {query}
Show Agentic RAG in four steps:
